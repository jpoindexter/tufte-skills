...
1. **Find the idea first.** Confections stand or fall on how deeply they illuminate ideas and the relations among them — name the argument before sketching the layout (§6, §7).
2. **Pick a structure (§2):** imagined scene, compartment grid, or both. Use *both* when you must show a unifying whole *and* itemized parts (Leviathan, Traps and Pitfalls).
3. **Gather heterogeneous image-events** — maps, photos, diagrams, drawings, real and invented — from different "strands." Mixing media is the form working, not a flaw.
4. **Show verbs, not just nouns** (§3): depict acts and consequences, not a static parts list.
5. **Make elements multifunction** (§2): one image earning several jobs is the signature of a good confection.
6. **Annotate for the stranger** (§5): integrate labels/captions; never require insider knowledge.
7. **Audit content share** (§8): keep substance dominant; strip chrome, decoration, and org-chart-shaped structure.
8. **Check for the cutter's art** (§7): did you select to illuminate, or to win? If to win, you have propaganda, not explanation.
